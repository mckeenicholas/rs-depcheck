...
static LANG_ATTR_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"lang\s*=\s*["']([^"']+)["']"#).unwrap()
});
...
pub fn parse_and_analyize<P: AsRef<Path>>(file_path: P) -> Result<ModuleAnalysis, Box<dyn Error>> {
    ...
    let parser = Parser::new(&parser_allocator, &source_to_parse, source_type);
    let ParserReturn {
        program,
        errors,
        panicked,
