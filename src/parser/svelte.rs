...
// Captures script tags, capturing attributes and content
...
impl DepParser for SvelteParser {
    fn parse<P: AsRef<Path>>(path: P) -> Result<Vec<String>, Box<dyn Error>> {
        let svelte_content = fs::read_to_string(path.as_ref())?;
        let mut aggregated_dependencies = HashSet::new();

        for script_cap in SCRIPT_TAG_REGEX.captures_iter(&svelte_content) {
